...
class RCTDBackend(SpatialBackend):
    ...
    def map(
        ...
        tmpdir = tempfile.mkdtemp(prefix="rctd_")
        try:
            ...
            if proc_result.returncode != 0:
                print(f"RCTD R stderr: {proc_result.stderr}")
                raise RuntimeError(f"RCTD R script failed: {proc_result.stderr}")
            ...
            if Path(tmpdir).exists():
                ...
                print(f"RCTD: Cleaned up temp directory {tmpdir}")
