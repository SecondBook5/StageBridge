...
class TrainingConfig:
    ...
    # Ablation flags
    freeze_encoder: bool = False  # Freeze encoder during transition phase
    no_niche: bool = False        # Use receiver only, no neighborhood context
    no_wes: bool = False          # Disable WES/genomic features
    deterministic: bool = False   # Direct prediction instead of flow matching
    use_prototypes: bool = False  # Prototype bottleneck for interpretability
    num_prototypes: int = 16      # Number of prototypes
    fusion_mode: str = "concat"   # Reference fusion: concat, attention, hlca_only, luca_only
    # Architectural ablations
    niche_encoder_type: str = "cross_attention"  # "cross_attention" or "self_attention" (pooled_niche uses self)
...
def create_model(
    ...
    try:
        ...
        model = StageBridgeV1Complete(
            ...
            # Ablation flags
            use_prototypes=config.use_prototypes,  # Prototype bottleneck for interpretability
            num_prototypes=config.num_prototypes,  # Number of learned prototypes
            niche_encoder_type=config.niche_encoder_type,  # cross_attention or self_attention
            use_hierarchical=config.use_hierarchical,  # Enable/disable hierarchical aggregation
        )
        ...
        if config.fusion_mode != "concat":
            ablation_flags.append(f"fusion={config.fusion_mode}")
        ...
        model_desc = f"StageBridgeV1Complete (HLCA={hlca_dim}d, LuCA={luca_dim}d)"
        if ablation_flags:
            model_desc += f" [ABLATIONS: {', '.join(ablation_flags)}]"
        log(f"Created {model_desc}")
    except ImportError:
...
def compute_deterministic_transition_loss(
    model: nn.Module,
    ...
    """Compute deterministic (direct prediction) transition loss.

    ABLATION: Tests whether flow matching helps vs direct endpoint prediction.

    Scientific question: "Does modeling the continuous flow field provide
    better transition predictions than directly predicting endpoints?"

    Instead of flow matching (sample t, predict velocity), this:
    1. Uses t=1 (endpoint)
    2. Predicts z_target directly from (z_source, context)
    3. Computes MSE(z_pred, z_target)

    Args:
        model: StageBridgeV1Complete model
        z_source: [B, D] source latent
        z_target: [B, D] target latent
        context: [B, context_dim] niche context
        wes_features: [B, wes_dim] WES features (optional)

    Returns:
        dict with loss_transition and predictions
    """
    ...
    # Use t=1 (endpoint) for deterministic prediction
    t = torch.ones(batch_size, 1, device=device)
    t_embed = actual_model.time_embedding(t)

    # Project WES features
    if wes_features is not None:
        wes_h = actual_model.wes_proj(wes_features)
    else:
        wes_h = torch.zeros(batch_size, actual_model.wes_hidden_dim, device=device)

    # Directly predict endpoint from source + context
    # At t=1, z_t = z_target, so we predict from z_source
    drift_input = torch.cat([z_source, context, t_embed, wes_h], dim=-1)
    z_pred = z_source + actual_model.drift_network(drift_input)  # Residual connection

    # Direct endpoint loss (not flow matching loss)
    loss_transition = torch.mean((z_pred - z_target) ** 2)

    return {
        "loss_transition": loss_transition,
        "drift_pred": z_pred - z_source,  # For compatibility with metrics
        "drift_true": z_target - z_source,
        "z_t": z_pred,
        "ot_cost": float("nan"),  # Not applicable
...
def apply_ablations_to_niche_tokens(
    ...
    config: TrainingConfig,
    hlca_embeddings: torch.Tensor | None = None,
    luca_embeddings: torch.Tensor | None = None,
) -> torch.Tensor:
    """Apply ablation modifications to niche tokens.

    This function implements the core ablation logic for scientific experiments:

    1. no_niche: Zero out ring tokens (1-4), keeping only receiver token.
       Scientific question: "Does local niche context improve prediction?"

    2. fusion_mode hlca_only: Zero out LuCA embedding in token 6.
       Scientific question: "Is the cancer reference (LuCA) necessary?"

    3. fusion_mode luca_only: Zero out HLCA embedding in token 5.
       Scientific question: "Is the healthy reference (HLCA) necessary?"

    Args:
        niche_tokens: [N, 9, D] token tensor (receiver, ring1-4, hlca, luca, pathway, stats)
        config: Training config with ablation flags
        hlca_embeddings: Original HLCA embeddings (for verification)
        luca_embeddings: Original LuCA embeddings (for verification)

    Returns:
        Modified niche_tokens with ablations applied
    """
    # Clone to avoid modifying original
    tokens = niche_tokens.clone()

    # =========================================================================
    # ABLATION: no_niche - Use receiver only, zero out neighborhood context
    # This is the CORE ablation testing our main hypothesis: niche matters
    # =========================================================================
    if config.no_niche:
        # Zero out ring tokens (indices 1-4)
        # Keep: token 0 (receiver), tokens 5-8 (hlca, luca, pathway, stats)
        # The model should fail to capture progression dynamics without niche
        tokens[:, 1:5, :] = 0.0
        log("  ABLATION [no_niche]: Zeroed ring tokens 1-4")

    # =========================================================================
    # ABLATION: fusion_mode - Single-reference ablations
    # Tests whether dual-reference geometry is necessary
    # =========================================================================
    if config.fusion_mode == "hlca_only":
        # Zero out LuCA token (index 6)
        # Hypothesis: Cancer-aware reference provides unique information
        tokens[:, 6, :] = 0.0
        log("  ABLATION [hlca_only]: Zeroed LuCA token 6")
    elif config.fusion_mode == "luca_only":
        # Zero out HLCA token (index 5)
        # Hypothesis: Healthy reference provides unique information
        tokens[:, 5, :] = 0.0
        log("  ABLATION [luca_only]: Zeroed HLCA token 5")
    ...
    if cells_path.exists() and neighborhoods_path.exists():
        try:
            ...
            if fused_cols:
                ...
                # ================================================================
                # APPLY ABLATIONS TO NICHE TOKENS (before train/val split)
                # This modifies the tokens based on ablation flags in config
                # ================================================================
                niche_tokens = apply_ablations_to_niche_tokens(
                    niche_tokens,
                    config,
                    hlca_embeddings=hlca_embeddings,
                    luca_embeddings=luca_embeddings,
                )
                ...
                if hasattr(actual_model, "transition_forward"):
                    ...
                    if stage_indices is not None:
                        ...
                        if can_transition.sum() >= 4:  # Need enough cells for OT
                            ...
                            # =========================================================
                            # ABLATION: deterministic - Direct endpoint prediction
                            # Tests whether flow matching helps vs direct prediction
                            # =========================================================
                            if config.deterministic:
...
def main():
    ...
    parser.add_argument(
        "--no_niche",
        action="store_true",
        help="Ablation: use receiver cell only, no neighborhood context",
    )
    parser.add_argument(
        "--no_wes",
        action="store_true",
        help="Ablation: disable WES/genomic features (tests evolutionary regularization)",
    )
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="Ablation: direct prediction instead of flow matching",
    )
    parser.add_argument(
        "--use_prototypes",
        action="store_true",
        help="Ablation: enable prototype bottleneck for interpretability",
    )
    parser.add_argument(
        "--num_prototypes",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Reference fusion mode (hlca_only/luca_only for single-reference ablations)",
    )
    # Architectural ablations
    parser.add_argument(
        "--niche_encoder_type",
        ...
        help="Ablation: niche encoder type (self_attention for pooled_niche-like behavior)",
    )
    parser.add_argument(
        "--no_hierarchical",
        action="store_true",
