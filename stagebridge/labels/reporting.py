...
def generate_label_repair_reports(
    *,
    ...
    refined_labels.loc[:, ["lesion_id", "patient_id", "donor_id", "stage", "edge_label", "progression_risk_score", "confidence_tier"]].to_csv(
    ...
    dataset_table = cleaned_manifest.groupby(["stage", "edge_label"], dropna=False, as_index=False).agg(
        n_lesions=("lesion_id", "nunique"),
        n_donors=("donor_id", "nunique"),
        n_wes_supported=("has_wes", lambda values: int(pd.Series(values).sum())),
    )
    dataset_table.to_csv(tables_root / "table1_cohort_composition_and_wes_availability.csv", index=False)
    cna_summary.to_csv(tables_root / "table2_cna_summary_by_lesion_and_stage.csv", index=False)
    phylo_table = clonal_summary.merge(
        phylogeny_summary[["lesion_id", "clone_sharing_score", "descendant_sharing_score", "tree_available"]],
        on="lesion_id",
        how="left",
    )
    phylo_table.to_csv(tables_root / "table3_clonal_phylogeny_summary.csv", index=False)
    edge_support[["edge_label", "positive_lesions", "negative_lesions", "uncertain_lesions", "excluded_lesions"]].to_csv(
    ...
    before = cleaned_manifest.groupby("edge_label", as_index=False)["original_label"].value_counts(dropna=False)
    if not before.empty:
        before["label_name"] = before["original_label"].map({1.0: "positive", 0.0: "negative"}).fillna("missing")
        ...
        axes[0].axis("off")
    after = edge_support.set_index("edge_label")[["positive_lesions", "negative_lesions", "uncertain_lesions", "excluded_lesions"]]
    if not after.empty:
    ...
    fig.tight_layout()
    fig.savefig(figures_root / "figure1_cohort_label_support_before_after.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    ...
    evidence_sources = [
        ...
        ("driver_proxy", cleaned_manifest.assign(driver_proxy=cleaned_manifest["availability_trace"].astype(str).str.contains("wes").astype(int))),
    ...
    fig.tight_layout()
    fig.savefig(figures_root / "figure2_evolutionary_evidence_summaries.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    ...
    if phylogeny_summary["tree_available"].fillna(False).astype(bool).any():
        ...
        fig.tight_layout()
        fig.savefig(figures_root / "figure3_patient_phylogeny_summary_panels.png", dpi=200, bbox_inches="tight")
        plt.close(fig)
    else:
        _save_placeholder(
    ...
    refined_labels["progression_risk_score"].plot(kind="hist", bins=12, ax=axes[0], title="Risk score distribution")
    refined_labels["refined_binary_label"].value_counts(dropna=False).plot(kind="bar", ax=axes[1], title="Refined label composition")
    ...
    fig.tight_layout()
    fig.savefig(figures_root / "figure4_refined_target_diagnostics.png", dpi=200, bbox_inches="tight")
    plt.close(fig)

    # Figure 5: split viability diagnostics.
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    edge_support.set_index("edge_label")[["positive_donors", "negative_donors"]].plot(kind="bar", ax=axes[0], title="Donor support by class")
    viability_plot = edge_support.set_index("edge_label")[["binary_viable", "continuous_viable"]].astype(int)
    viability_plot.plot(kind="bar", ax=axes[1], title="Target viability")
    fig.suptitle("Figure 5. Split viability diagnostics")
    fig.tight_layout()
    fig.savefig(figures_root / "figure5_split_viability_diagnostics.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
