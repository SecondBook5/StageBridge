...
def summarize_fixed_points(model: object, x_state: Tensor, *, context: Tensor, edge_id: int) -> dict[str, float]:
    edge_ids = torch.full((x_state.shape[0],), int(edge_id), dtype=torch.long, device=x_state.device)
