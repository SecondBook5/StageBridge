"""Coupling utilities for OT-informed flow matching.

Provides helpers for building cost matrices and couplings with
optional extra cost terms (e.g., spatial distance regularization).
"""
...
def build_cost_matrix(
    ...
    *,
    ...
    """Build transport cost matrix with optional regularization costs.

    Args:
        x_src: [N, D] source points
        x_tgt: [M, D] target points
        extra_cost: [N, M] optional additional cost term
    ...
    if extra_cost is not None:
        if extra_cost.shape != cost.shape:
            raise ValueError(
                f"extra_cost shape {tuple(extra_cost.shape)} does not match "
                f"cost shape {tuple(cost.shape)}"
            )
...
def build_sinkhorn_coupling_from_cost(
    ...
    *,
    ...
    """Build OT coupling from precomputed cost matrix.
...
def build_ot_coupling(
    ...
    *,
    ...
    """Build OT coupling with optional regularization-aware costs.

    Convenience function that builds cost matrix and computes coupling.
    ...
    Args:
        ...
        epsilon: Entropic regularization
        n_iters: Sinkhorn iterations
        extra_cost: [N, M] optional additional cost
