...
def plot_radar_chart(
    ...
    for idx, (label, vals) in enumerate(zip(labels, values)):
        ...
        ax.plot(angles, vals_plot, "o-", linewidth=2, label=label, color=colors[idx], alpha=0.7)
    ...
    legend = ax.legend(loc="upper right", bbox_to_anchor=(1.3, 1.0), framealpha=0.95, fontsize=10)
...
def plot_correlation_matrix(
    ...
    # Draw heatmap
    im = ax.imshow(corr, cmap="RdBu_r", aspect="auto", vmin=-1, vmax=1, interpolation="nearest")
...
def plot_ridge_distributions(
    ...
    fig, axes = plt.subplots(
        n_distributions, 1, figsize=(11, 2 * n_distributions), sharex=True, dpi=150
    )
    fig.patch.set_facecolor("white")
    ...
    for idx, (label, data) in enumerate(data_dict.items()):
        ...
        if len(data_clean) > 0:
            ax.hist(
                data_clean, bins=50, density=True, alpha=0.6, color=colors[idx], edgecolor="white"
            )
            ...
            try:
                ...
                ax.plot(x_range, kde(x_range), color=colors[idx], linewidth=2.5, alpha=0.9)
            except Exception:
                pass

        # Styling
        ax.set_ylabel(label, fontsize=11, fontweight="bold", rotation=0, ha="right", va="center")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
