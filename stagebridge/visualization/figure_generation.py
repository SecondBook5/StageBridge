...
def generate_figure1_architecture(output_path):
    ...
    for layer in layers:
        rect = plt.Rectangle((0.15, layer["y"]), 0.7, layer["h"],
                            facecolor=layer["color"], edgecolor="white",
                            linewidth=3, alpha=0.85, zorder=2)
        ax.add_patch(rect)
        ax.text(0.5, layer["y"] + layer["h"]/2, layer["name"],
    ...
    ax.text(0.5, 0.98, "Input: Cell Latents + Spatial Context + Genomics",
           ha="center", fontsize=11, bbox=dict(boxstyle="round", facecolor="#ecf0f1", alpha=0.8))
    ax.text(0.5, 0.03, "Output: Transition Dynamics + Attention Patterns",
...
def generate_figure5_attention_patterns(model, test_loader, output_path):
    ...
    bars = ax.bar(x, ring_attn, color=['#e74c3c', '#e67e22', '#f39c12', '#f1c40f'],
    ...
    ax.text(0.5, 0.5, insight_text, ha='center', va='center',
...
def generate_figure3_niche_influence_biology(influence_df, pathway_df, cells_df, output_path):
    ...
    stage_influence = stage_influence.reindex([s for s in stage_order if s in stage_influence.index])
    colors = ['#3498db', '#2ecc71', '#f39c12', '#e74c3c'][:len(stage_influence)]
    bars = ax.bar(range(len(stage_influence)), stage_influence.values, color=colors,
                  edgecolor='black', linewidth=2, alpha=0.85)
    ...
    ax = axes[0, 1]
    ax.scatter(merged['caf_score'], merged['ring_influence'], alpha=0.5, s=30, c=merged['emt_score'],
    ...
    ax = axes[1, 1]
    stage_data = [merged[merged['stage'] == s]['ring_influence'].values
                  for s in stage_influence.index if s in merged['stage'].values]
    parts = ax.violinplot(stage_data, positions=range(len(stage_influence)),
    ...
    ax.text(0.5, 0.5, findings, ha='center', va='center',
           fontsize=11, transform=ax.transAxes, fontweight='bold',
           bbox=dict(boxstyle='round', facecolor='#f1c40f', edgecolor='#34495e',
                    linewidth=2, alpha=0.95))
...
def generate_figure8_flagship_biology(cells_df, influence_df, pathway_df, output_path):
    ...
    positions = [1, 2, 3]
    bp = ax.boxplot([tertile_influence['Low'], tertile_influence['Medium'], tertile_influence['High']],
    ...
    # B: Influence vs EMT
    ax = axes[0, 1]
    scatter = ax.scatter(merged['ring_influence'], merged['emt_score'],
                        c=merged['caf_score'], cmap='RdYlBu_r', s=50,
    ...
    for i in range(4):
        for j in range(4):
            text = ax.text(j, i, f'{sig_corr.iloc[i, j]:.2f}',
    ...
    ax.text(0.5, 0.5, summary, ha='center', va='center',
           fontsize=11, transform=ax.transAxes, fontweight='bold',
           bbox=dict(boxstyle='round', facecolor='#2ecc71', edgecolor='#27ae60',
...
def generate_figure2_dimensionality_reduction(cells_df, output_path):
    ...
    # Plot first two PCs
    scatter = ax.scatter(Z_pca[:, 0], Z_pca[:, 1], c=colors_stage, cmap='tab10',
                        s=30, alpha=0.6, edgecolors='black', linewidth=0.3)
    ax.set_xlabel(f'PC1 ({100*pca.explained_variance_ratio_[0]:.1f}%)', fontweight='bold', fontsize=11)
    ax.set_ylabel(f'PC2 ({100*pca.explained_variance_ratio_[1]:.1f}%)', fontweight='bold', fontsize=11)
    ...
    ax.text(n_80, 0.5, f'{n_80} dims\n(80%)', ha='center', fontsize=9,
           bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
    ax.text(n_90, 0.5, f'{n_90} dims\n(90%)', ha='center', fontsize=9,
    ...
    scatter = ax.scatter(Z_tsne[:, 0], Z_tsne[:, 1], c=colors_tsne, cmap='tab10',
                        s=30, alpha=0.6, edgecolors='black', linewidth=0.3)
    ...
    scatter = ax.scatter(Z_umap[:, 0], Z_umap[:, 1], c=colors_umap, cmap='tab10',
                        s=30, alpha=0.6, edgecolors='black', linewidth=0.3)
    ...
    try:
        ...
        scatter = ax.scatter(Z_phate[:, 0], Z_phate[:, 1], c=colors_phate, cmap='tab10',
                            s=30, alpha=0.6, edgecolors='black', linewidth=0.3)
        ...
        scatter = ax.scatter(Z_pca[:, 0], Z_pca[:, 1], c=colors_tmb, cmap='viridis',
                            s=30, alpha=0.6, edgecolors='black', linewidth=0.3)
        ax.set_xlabel(f'PC1 ({100*pca.explained_variance_ratio_[0]:.1f}%)', fontweight='bold', fontsize=11)
        ax.set_ylabel(f'PC2 ({100*pca.explained_variance_ratio_[1]:.1f}%)', fontweight='bold', fontsize=11)
...
def generate_figure4_model_performance(training_results_df, baseline_results=None, output_path=None):
    ...
    if 'epoch' in training_results_df.columns and 'train_loss' in training_results_df.columns:
        for fold in training_results_df['fold'].unique():
            ...
            ax.plot(fold_data['epoch'], fold_data['train_loss'],
                   alpha=0.5, linewidth=2, label=f'Fold {fold}')
    ...
    if all(m in training_results_df.columns for m in metrics):
        ...
        ax.plot(x, mean_values, 'ko-', linewidth=3, markersize=10,
    ...
    for model, color in zip(models, colors):
        ...
        ax.plot(fpr_base, tpr, color=color, lw=lw,
    ...
    for model, color in zip(models, colors):
        ...
        lw = 3 if model == 'StageBridge' else 2
        ax.plot(recall_base, precision, color=color, lw=lw,
    ...
    for i in range(4):
        for j in range(5):
            text = ax.text(j, i, f'{comparison_metrics[i, j]:.2f}',
    ...
    improvement = (comparison_metrics[0, 0] - comparison_metrics[1, 0]) / comparison_metrics[1, 0] * 100
    ...
    ax.text(0.5, 0.5, insights, ha='center', va='center',
           fontsize=11, transform=ax.transAxes, fontweight='bold',
           bbox=dict(boxstyle='round', facecolor='#2ecc71', edgecolor='#27ae60',
                    linewidth=3, alpha=0.95))
...
def generate_figure6_spatial_benchmark(benchmark_results, output_path):
    ...
    bars = ax.barh(backends, metrics_df['mapping_quality'], color=colors, edgecolor='black', linewidth=2)
    ...
    bars = ax.barh(backends, metrics_df['runtime_minutes'], color=colors, edgecolor='black', linewidth=2)
    ...
    # C: Memory Usage
    ax = fig.add_subplot(gs[0, 2])
    bars = ax.bar(range(n_backends), metrics_df['memory_gb'], color=colors,
                  edgecolor='black', linewidth=2, alpha=0.85)
    ...
    for i, bar in enumerate(bars):
        ...
        ax.text(bar.get_x() + bar.get_width()/2, height + 0.5,
               f'{height:.1f}GB', ha='center', fontsize=10, fontweight='bold')

    # D: Downstream Utility
    ax = fig.add_subplot(gs[1, 0])
    bars = ax.barh(backends, metrics_df['downstream_utility'], color=colors, edgecolor='black', linewidth=2)
    ...
    normalized_data['runtime_minutes'] = 1 - (normalized_data['runtime_minutes'] / normalized_data['runtime_minutes'].max())
    ...
    scatter = ax.scatter(metrics_df['runtime_minutes'], metrics_df['mapping_quality'],
                        s=metrics_df['memory_gb'] * 50, c=metrics_df['downstream_utility'],
                        cmap='RdYlGn', edgecolors='black', linewidths=2, alpha=0.8,
    ...
    # Add size legend
    ax.text(0.02, 0.98, 'Bubble size = Memory', transform=ax.transAxes,
...
def generate_flow_matching_dynamics(model, test_loader, cells_df, output_path):
    ...
    if stages is not None:
        stage_labels = pd.Categorical(stages)
        colors_stage = stage_labels.codes
        scatter = ax.scatter(Z_2d[:, 0], Z_2d[:, 1], c=colors_stage, cmap='viridis',
        ...
        ax.scatter(Z_2d[:, 0], Z_2d[:, 1], c='steelblue', s=50, alpha=0.7,
                  edgecolors='black', linewidth=0.5)

    ax.set_xlabel(f'Latent Dim 1 ({100*pca.explained_variance_ratio_[0]:.1f}%)',
                 fontweight='bold', fontsize=11)
    ax.set_ylabel(f'Latent Dim 2 ({100*pca.explained_variance_ratio_[1]:.1f}%)',
                 fontweight='bold', fontsize=11)
    ...
    ax.scatter([], [], c='green', s=100, marker='o', edgecolors='black',
              linewidth=2, label='Start')
    ax.scatter([], [], c='red', s=100, marker='*', edgecolors='black',
    ...
    im = ax.imshow(velocity_mag, extent=[x_min, x_max, y_min, y_max],
                   origin='lower', cmap='hot', aspect='auto', alpha=0.8)
    ax.contour(xx, yy, velocity_mag, levels=5, colors='black',
    ...
    plt.suptitle("Flow Matching & Stochastic Transition Dynamics",
...
def generate_set_transformer_mechanics(model, test_loader, output_path):
    ...
    positions = {
        'Receiver': (0.5, 0.8),
        'Ring1': (0.2, 0.6), 'Ring2': (0.4, 0.6), 'Ring3': (0.6, 0.6), 'Ring4': (0.8, 0.6),
        'HLCA': (0.25, 0.35), 'LuCA': (0.5, 0.35),
        'Pathway': (0.75, 0.35),
        'Stats': (0.5, 0.1)
    }

    colors_token = {
        'Receiver': '#e74c3c', 'Ring1': '#3498db', 'Ring2': '#3498db',
        'Ring3': '#3498db', 'Ring4': '#3498db', 'HLCA': '#2ecc71',
        'LuCA': '#2ecc71', 'Pathway': '#f39c12', 'Stats': '#9b59b6'
    }

    # Draw tokens
    for token, (x, y) in positions.items():
        circle = plt.Circle((x, y), 0.06, color=colors_token[token],
                           alpha=0.8, edgecolor='black', linewidth=2)
        ax.add_patch(circle)
        ax.text(x, y, token.split('Ring')[-1] if 'Ring' in token else token[:4],
    ...
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#e74c3c',
                  markersize=12, label='Receiver (query)'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#3498db',
                  markersize=12, label='Spatial Rings (1-4)'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#2ecc71',
                  markersize=12, label='References (HLCA/LuCA)'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#f39c12',
                  markersize=12, label='Pathway Context'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#9b59b6',
                  markersize=12, label='Statistics'),
    ]
    ...
    for i in range(n_inputs):
        x = (i + 0.5) / n_inputs
        circle = plt.Circle((x, input_y), 0.03, color='#3498db',
    ...
    for i in range(n_inducing):
        x = (i + 1) / (n_inducing + 1)
        circle = plt.Circle((x, inducing_y), 0.04, color='#e74c3c',
                           alpha=0.8, edgecolor='black', linewidth=2)
        ax.add_patch(circle)
    ...
    for i in range(n_inputs):
        x = (i + 0.5) / n_inputs
        circle = plt.Circle((x, output_y), 0.03, color='#2ecc71',
                           alpha=0.7, edgecolor='black', linewidth=1.5)
        ax.add_patch(circle)
    ...
    ax.text(0.05, inducing_y, 'Inducing\nPoints', fontsize=9, fontweight='bold', va='center',
           color='#e74c3c')
    ...
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.set_title('B. ISAB: Complexity Reduction (O(n²) → O(nm))',
    ...
    bars = ax.bar(x_pos, pool_weights, color='#3498db', alpha=0.7,
    ...
    ax.axhline(1/n_tokens, color='gray', linestyle='--', linewidth=2,
              label='Uniform')
    ax.legend(fontsize=10)
    ...
    plt.suptitle("Set Transformer Architecture & Hierarchical Information Flow",
                fontsize=18, fontweight="bold", y=0.98)
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches="tight", facecolor='white')
...
def generate_ablation_impact_visualization(ablation_results_df, output_path):
    ...
    bars = ax.bar(range(len(model_names)), aucs, color=colors, alpha=0.8,
                  edgecolor='black', linewidth=2)

    # Add drop annotations
    for i in range(1, len(drops)):
        ax.annotate('', xy=(i, full_auc), xytext=(i, aucs[i]),
                   arrowprops=dict(arrowstyle='<->', color='red', lw=2))
        ax.text(i, (full_auc + aucs[i])/2, f'-{drops[i]:.2f}',
    ...
    ax.set_ylim(0.7, 1.0)
    ax.axhline(full_auc, color='green', linestyle='--', linewidth=2,
    ...
    bars = ax.barh(components_sorted, values_sorted, color=colors_comp,
                   alpha=0.8, edgecolor='black', linewidth=2)
    ...
    for i in range(len(model_names)):
        for j in range(len(metrics)):
            text = ax.text(j, i, f'{metric_data[i, j]:.2f}',
                          ha="center", va="center", color="black",
                          fontsize=9, fontweight='bold')
    ...
    for i, layer in enumerate(layers):
        # Draw layer box
        rect = plt.Rectangle((0.2, layer['y'] - 0.05), 0.3, 0.08,
                            facecolor=ablations[list(ablations.keys())[i+1]]['color'],
                            edgecolor='black', linewidth=2, alpha=0.7)
        ax.add_patch(rect)
        ax.text(0.35, layer['y'], layer['name'], ha='center', va='center',
        ...
        ax.arrow(0.75, layer['y'], arrow_len, 0, head_width=0.02, head_length=0.03,
                fc='red', ec='red', alpha=0.7)
        ax.text(0.75 + arrow_len + 0.05, layer['y'], f'-{impact:.2f}',
    ...
    scatter = ax.scatter(n_components, aucs, s=[200 if m == 'Full Model' else 150 for m in model_names],
                        c=colors, edgecolors='black', linewidth=2, alpha=0.8)

    for i, name in enumerate(model_names):
        if name != 'Full Model':
            ax.annotate(name, (n_components[i], aucs[i]),
                       xytext=(5, 5), textcoords='offset points', fontsize=7)

    # Highlight full model
    full_idx = 0
    ax.annotate('Full Model', (n_components[full_idx], aucs[full_idx]),
    ...
    for i in range(n_comp):
        for j in range(n_comp):
            if i != j:
                text = ax.text(j, i, f'{synergy[i, j]:.2f}',
    ...
    bars = ax.bar(range(len(model_names)), significance, color=colors,
                  alpha=0.8, edgecolor='black', linewidth=2)

    # Add significance lines
    ax.axhline(-np.log10(0.05), color='orange', linestyle='--', linewidth=2,
              label='p=0.05', alpha=0.7)
    ax.axhline(-np.log10(0.01), color='red', linestyle='--', linewidth=2,
    ...
    plt.suptitle("Comprehensive Ablation Study: Component Contributions",
                fontsize=18, fontweight="bold", y=0.98)
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches="tight", facecolor='white')
...
def generate_cross_modal_integration(cells_df, output_path):
    ...
    for mod, data in modalities.items():
        circle = plt.Circle(data['pos'], data['size'], color=data['color'],
                           alpha=0.8, edgecolor='black', linewidth=3)
        ax.add_patch(circle)
        ax.text(data['pos'][0], data['pos'][1], mod, ha='center', va='center',
               fontsize=11, fontweight='bold', color='white')

    # Draw integration arrows
    connections = [
        ('snRNA-seq', 'Fused'), ('Spatial', 'Fused'), ('WES', 'Fused'),
        ('HLCA', 'Fused'), ('LuCA', 'Fused'),
        ('snRNA-seq', 'HLCA'), ('snRNA-seq', 'LuCA'),
    ]
    ...
    scatter = ax.scatter(spatial_dist, expr_similarity, c=expr_similarity,
    ...
    bars = ax.bar(range(len(tmb_bins)), transition_prob, yerr=errors,
                  color=['#3498db', '#f39c12', '#e74c3c'],
                  alpha=0.8, edgecolor='black', linewidth=2, capsize=10)
    ...
    bars1 = ax.bar(x - width/2, hlca_score, width, label='HLCA (Healthy)',
                   color='#9b59b6', alpha=0.8, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x + width/2, luca_score, width, label='LuCA (Cancer)',
    ...
    bars = ax.barh(strategies, performance, color=colors_strat,
                   alpha=0.8, edgecolor='black', linewidth=2)
    ...
    scatter = ax.scatter(Z_2d[:, 0], Z_2d[:, 1], c=colors_z, cmap='RdYlBu_r',
    ...
    ax.set_ylabel(f'PC2 ({100*pca.explained_variance_ratio_[1]:.1f}%)',
                 fontweight='bold', fontsize=11)
    ax.set_title('G. Integrated Latent Representation (Colored by TMB)',
                fontweight='bold', fontsize=13)
    ax.grid(alpha=0.3, linestyle='--')
    plt.colorbar(scatter, ax=ax, label='TMB')
    ...
    wedges, texts, autotexts = ax.pie(info_contribution, labels=modalities_info,
                                       autopct='%1.1f%%', colors=colors_info,
    ...
    plt.suptitle("Cross-Modal Data Integration & Fusion",
                fontsize=18, fontweight="bold", y=0.98)
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches="tight", facecolor='white')
