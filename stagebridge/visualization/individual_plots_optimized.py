...
def plot_pca_with_variance(embeddings: np.ndarray, labels: np.ndarray,
    ...
    for i, label in enumerate(unique_labels):
        ...
        plt.scatter(X_pca[mask, 0], X_pca[mask, 1],
                   c=[colors[i]], label=f'Stage {label}',
    ...
    plt.title(f'PCA (Total variance: {variance_ratio[:2].sum()*100:.1f}%)',
...
def plot_phate(embeddings: np.ndarray, labels: np.ndarray,
              output_path: Path, dpi: int = 300):
    ...
    for i, label in enumerate(unique_labels):
        mask = labels == label
        plt.scatter(X_phate[mask, 0], X_phate[mask, 1],
                   c=[colors[i]], label=f'Stage {label}',
                   alpha=0.6, s=50, edgecolors='white', linewidth=0.5)
...
def plot_roc_curve(fpr: np.ndarray, tpr: np.ndarray, auc_score: float,
    ...
    plt.plot(fpr, tpr, linewidth=3, label=f'ROC (AUC = {auc_score:.3f})',
            color='#2ecc71')
...
def plot_pr_curve(precision: np.ndarray, recall: np.ndarray, ap_score: float,
                 output_path: Path, dpi: int = 300):
    """Individual Precision-Recall curve plot"""
    plt.figure(figsize=(8, 8))

    plt.plot(recall, precision, linewidth=3, label=f'PR (AP = {ap_score:.3f})',
            color='#9b59b6')
...
def plot_attention_heatmap(attention: np.ndarray, token_labels: list,
                           output_path: Path, dpi: int = 300):
...
def generate_all_plots_parallel(embeddings: np.ndarray, labels: np.ndarray,
                                output_dir: Path, dpi: int = 300,
                                max_workers: int = 4):
    """
