...
def generate_figure2_dimensionality_reduction(
    ...
    ax.set_title(
        f"PCA (var: {pca.explained_variance_ratio_[:2].sum() * 100:.1f}%)",
        fontsize=12,
        fontweight="bold",
    )
    ...
    ax.plot(range(1, min(21, len(variance) + 1)), variance[:20], "o-", linewidth=2, markersize=6)
...
def generate_figure4_model_performance(
    training_history: dict, test_metrics: dict, output_path: Path
):
    """
    ...
    if "train_loss" in training_history:
        ...
        ax.plot(
            epochs, training_history["train_loss"], "o-", label="Train", linewidth=2, markersize=4
        )
        if "val_loss" in training_history:
            ax.plot(
                epochs, training_history["val_loss"], "s-", label="Val", linewidth=2, markersize=4
            )
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
    ...
    if "fpr" in test_metrics and "tpr" in test_metrics:
        ax.plot(
            ...
            test_metrics["tpr"],
            linewidth=3,
            label=f"AUC = {test_metrics.get('roc_auc', 0):.3f}",
        )
    ...
    if "precision" in test_metrics and "recall" in test_metrics:
        ax.plot(
            ...
            linewidth=3,
            label=f"AP = {test_metrics.get('average_precision', 0):.3f}",
        )
    ...
    if "train_acc" in training_history:
        ...
        ax.plot(
            epochs, training_history["train_acc"], "o-", label="Train", linewidth=2, markersize=4
        )
        if "val_acc" in training_history:
            ax.plot(
                epochs, training_history["val_acc"], "s-", label="Val", linewidth=2, markersize=4
            )
        ax.set_xlabel("Epoch")
    ...
    if "accuracy" in test_metrics:
        ...
        bars = ax.bar(
            metrics.keys(), metrics.values(), color=COLORS["performance"][: len(metrics)]
        )
        ax.set_ylim([0, 1])
...
def generate_figure5_attention_heatmap(
    ...
    table = ax.table(cellText=stats_data, cellLoc="left", bbox=[0, 0, 1, 1], edges="horizontal")
