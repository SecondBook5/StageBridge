"""PanIN feature ingestion and registration.

Loads source-derived feature matrices into ``PanINFeatureBlock`` objects and
...
feature dropping. Metadata columns (stage/donor/sample/section/platform/split/
target/edge/outcome) and coordinates are rejected from feature vectors.

Milestone-8 readers cover CSV/TSV (unit-test fixtures and simple source tables).
Heavy source formats (e.g. rds/anndata) are read lazily only if the real source
requires them; those imports never happen at module import time.
"""
...
from .config import PanINFeatureBlockConfig
...
#: Feature ids that must never be treated as molecular features.
FORBIDDEN_FEATURE_ID_TOKENS = frozenset(
    {
        "stage", "grade", "donor", "patient", "sample", "section", "platform",
        "split", "fold", "target_state", "transition_edge", "edge", "outcome",
...
def _assert_feature_ids_are_molecular(feature_ids: tuple[str, ...]) -> None:
    for fid in feature_ids:
        if fid.strip().lower() in FORBIDDEN_FEATURE_ID_TOKENS:
            raise CCRTValidationError(
                f"feature id '{fid}' is metadata/coordinate, not a molecular "
                "feature; it must not enter a feature vector"
            )
...
def load_panin_feature_block(
    config: PanINFeatureBlockConfig,
    *,
    source_root: str | Path,
) -> PanINFeatureBlock:
    """Load a feature block, preserving configured feature order."""
    _assert_feature_ids_are_molecular(config.feature_ids)
    ...
    if suffix in (".csv", ".tsv", ".txt"):
        ...
        # Heavy source formats are read lazily by a dedicated helper; unavailable
        # in the Milestone-8 local environment. Fail clearly rather than guess.
        raise CCRTValidationError(
...
def register_panin_feature_spaces(
    blocks: list[PanINFeatureBlock],
