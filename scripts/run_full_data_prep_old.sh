...
echo ""
echo "[1/26] Running main data prep with LIANA..."
if skip_if_exists "$CANONICAL/liana_interactions.parquet" "LIANA"; then
    echo "  Skipping step 1 (LIANA results exist)"
else
...
# 2. SCENIC - SKIPPED (run separately with scripts/run_scenic.sh)
echo ""
echo "[2/26] SCENIC regulon analysis..."
echo "  SKIP: Run separately with: bash scripts/run_scenic.sh"
echo "  (pySCENIC has environment conflicts - use dedicated env)"

# 3. Squidpy spatial statistics
echo ""
echo "[3/26] Running Squidpy spatial statistics..."
if skip_if_exists "$CANONICAL/spatial_stats/nhood_enrichment.parquet" "Squidpy"; then
...
echo ""
echo "[4/26] Running differential expression analysis..."
if skip_if_exists "$CANONICAL/de_analysis/de_stage_Normal.parquet" "DE"; then
