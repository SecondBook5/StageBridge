...
def fig_training_baselines(results_dir: Path, output_path: Path):
    """Generate combined training figure with 4 panels: SSL (train+val), Transition (train+val), Baselines, Final bar."""
    fig = plt.figure(figsize=(14, 8))
    gs = GridSpec(2, 2, height_ratios=[1, 1], width_ratios=[1, 1], hspace=0.3, wspace=0.25)

    fold_colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']
    SSL_EPOCHS = 50  # SSL pretraining is epochs 0-49, transition is 50+

    # Load all histories by fold
    histories_by_fold = {}
    for metrics_path in sorted(results_dir.glob("full/fold_*/seed_*/logs/metrics.json")):
        parts = str(metrics_path).split('/')
        fold = [p for p in parts if p.startswith('fold_')][0]
        with open(metrics_path) as f:
            data = json.load(f)
        history = data.get('history', {})
        if fold not in histories_by_fold:
            histories_by_fold[fold] = []
        histories_by_fold[fold].append(history)

    # Panel A: SSL Pretraining (train + val)
    ax1 = fig.add_subplot(gs[0, 0])
    if histories_by_fold:
        for i, (fold, histories) in enumerate(sorted(histories_by_fold.items())):
            color = fold_colors[i % len(fold_colors)]
            for hist in histories:
                train_ssl = hist.get('train_loss', [])[:SSL_EPOCHS]
                val_ssl = hist.get('val_loss', [])[:SSL_EPOCHS]
                if train_ssl:
                    epochs = np.arange(len(train_ssl))
                    ax1.plot(epochs, train_ssl, color=color, alpha=0.3, lw=1, ls='--')
                if val_ssl:
                    epochs = np.arange(len(val_ssl))
                    ax1.plot(epochs, val_ssl, color=color, alpha=0.7, lw=1.5)
            ax1.plot([], [], color=color, label=fold.replace('_', ' ').title(), lw=2)
        ax1.set_yscale('log')
        ax1.legend(frameon=False, loc='upper right', fontsize=7, ncol=2)
        # Add train/val legend
        ax1.plot([], [], color='gray', ls='--', alpha=0.5, label='Train')
        ax1.plot([], [], color='gray', ls='-', alpha=0.8, label='Val')
    else:
        ax1.text(0.5, 0.5, 'No SSL data found', ha='center', va='center', transform=ax1.transAxes)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Loss (log scale)')
    ax1.set_title('A. SSL Pretraining', fontweight='bold')

    # Panel B: Transition Training (train + val)
    ax2 = fig.add_subplot(gs[0, 1])
    if histories_by_fold:
        for i, (fold, histories) in enumerate(sorted(histories_by_fold.items())):
            color = fold_colors[i % len(fold_colors)]
            for hist in histories:
                train_trans = hist.get('train_loss', [])[SSL_EPOCHS:]
                val_trans = hist.get('val_loss', [])[SSL_EPOCHS:]
                if train_trans:
                    epochs = np.arange(len(train_trans))
                    ax2.plot(epochs, train_trans, color=color, alpha=0.3, lw=1, ls='--')
                if val_trans:
                    epochs = np.arange(len(val_trans))
                    ax2.plot(epochs, val_trans, color=color, alpha=0.7, lw=1.5)
            ax2.plot([], [], color=color, label=fold.replace('_', ' ').title(), lw=2)
        ax2.legend(frameon=False, loc='upper right', fontsize=7, ncol=2)
    else:
        ax2.text(0.5, 0.5, 'No transition data found', ha='center', va='center', transform=ax2.transAxes)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Loss (MSE)')
    ax2.set_title('B. Transition Training', fontweight='bold')

    # Load baseline histories
    baseline_histories = {}
    for baseline_path in sorted(results_dir.glob("baselines/*/fold_*/seed_*/baseline_*.json")):
        with open(baseline_path) as f:
            data = json.load(f)
        baseline = data.get('baseline', 'unknown')
        history = data.get('history', {})
        if history:
            if baseline not in baseline_histories:
                baseline_histories[baseline] = []
            baseline_histories[baseline].append(history)

    # Panel C: Baseline comparison curves (StageBridge transition vs baselines)
    ax3 = fig.add_subplot(gs[1, 0])

    has_curves = False
    # StageBridge transition mean
    if histories_by_fold:
        has_curves = True
        all_val = []
        for histories in histories_by_fold.values():
            for hist in histories:
                val_trans = hist.get('val_loss', [])[SSL_EPOCHS:]
                if val_trans:
                    all_val.append(val_trans)
        if all_val:
            max_len = max(len(v) for v in all_val)
            ...
            epochs = np.arange(len(val_mean))
            ax3.plot(epochs, val_mean, label='StageBridge', lw=2.5, color=BASELINE_COLORS['stagebridge'])

    if baseline_histories:
        has_curves = True
        for baseline, all_hist in baseline_histories.items():
            all_val = [h.get('val_loss', []) for h in all_hist if h.get('val_loss')]
            if all_val:
                max_len = max(len(v) for v in all_val)
                val_mean = np.zeros(max_len)
                counts = np.zeros(max_len)
                ...
                ax3.plot(epochs, val_mean, label=label, lw=2, color=color, alpha=0.8)

    if has_curves:
        ax3.legend(frameon=False, loc='upper right', fontsize=7)
        ax3.set_ylim(0, 0.22)
    else:
        ax3.text(0.5, 0.5, 'No baseline data found', ha='center', va='center', transform=ax3.transAxes)
    ax3.set_xlabel('Epoch')
    ax3.set_ylabel('Validation Loss')
    ax3.set_title('C. StageBridge vs Baselines', fontweight='bold')

    # Panel D: Final validation loss bar chart
    ...
    if report_path.exists():
        ...
        models = ['StageBridge']
        values = [report.get('full_model', {}).get('mean_val_loss', 0)]
        colors = [BASELINE_COLORS['stagebridge']]

        for baseline in ['graphsage', 'pooling', 'deepsets', 'set_transformer']:
            if baseline in report.get('baselines', {}):
                models.append(baseline.replace('_', ' ').title())
                values.append(report['baselines'][baseline].get('mean_val_loss', 0))
                colors.append(BASELINE_COLORS.get(baseline, 'gray'))

        x = np.arange(len(models))
        bars = ax4.bar(x, values, color=colors, edgecolor='black', lw=1.5)
        ax4.set_xticks(x)
        ax4.set_xticklabels(models, rotation=45, ha='right')
        ax4.set_ylabel('Validation Loss (MSE)')
        ...
        ax4.text(0.5, 0.5, 'No comparison report found', ha='center', va='center', transform=ax4.transAxes)
...
def fig_ablations(results_dir: Path, output_path: Path):
    """Generate ablation study figure using violin plots."""
    ...
    if full_model and 'all_losses' in full_model:
        for loss in full_model['all_losses']:
            rows.append({'Configuration': 'Full Model', 'Validation Loss': loss, 'type': 'full'})

    # Add ablations sorted by delta
    sorted_ablations = sorted(
        [(k, v) for k, v in ablations.items()],
    ...
    for abl_name, abl_data in sorted_ablations:
        pretty_name = name_map.get(abl_name, abl_name.replace('_', ' ').title())
        delta = abl_data.get('delta_vs_full', 0)
        label = f"{pretty_name}\n({delta:+.1f}%)"
        for loss in abl_data.get('all_losses', []):
            rows.append({'Configuration': label, 'Validation Loss': loss, 'type': 'ablation'})
    ...
    # Color palette: green for full, gradient red for ablations based on delta
    order = ['Full Model'] + [f"{name_map.get(a[0], a[0].replace('_', ' ').title())}\n({a[1].get('delta_vs_full', 0):+.1f}%)" for a in sorted_ablations]

    # Create color list
    colors = ['#27ae60']  # Full model green
    for abl_name, abl_data in sorted_ablations:
        delta = abl_data.get('delta_vs_full', 0)
        if delta > 8:
            ...
            colors.append('#3498db')  # Blue for improvement

    palette = dict(zip(order, colors))

    # Violin plot
    sns.violinplot(
        data=df, x='Configuration', y='Validation Loss',
        order=order, palette=palette, ax=ax1,
    ...
    sns.stripplot(
        data=df, x='Configuration', y='Validation Loss',
        order=order, color='white', edgecolor='black',
        size=6, linewidth=1, ax=ax1, alpha=0.8
    )

    # Add full model reference line
    if full_model and 'mean_val_loss' in full_model:
        ax1.axhline(full_model['mean_val_loss'], color='#27ae60', ls='--', lw=2, alpha=0.7,
                   label=f"Full model mean: {full_model['mean_val_loss']:.4f}")
        ax1.legend(loc='upper right', frameon=True, facecolor='white')

    ax1.set_xlabel('')
    ax1.set_ylabel('Validation Loss (MSE)', fontweight='bold')
    ax1.set_title('A. Ablation Study: Validation Loss Distribution', fontweight='bold', fontsize=12)
    ax1.tick_params(axis='x', rotation=45)
    plt.setp(ax1.get_xticklabels(), ha='right')

    # Panel B: Delta summary bar chart
    ax2 = fig.add_subplot(gs[1])

    names = [name_map.get(a[0], a[0].replace('_', ' ').title()) for a in sorted_ablations]
    deltas = [a[1].get('delta_vs_full', 0) for a in sorted_ablations]
    bar_colors = colors[1:]  # Skip full model color

    y_pos = np.arange(len(names))
    bars = ax2.barh(y_pos, deltas, color=bar_colors, edgecolor='black', lw=1)

    ax2.axvline(0, color='black', lw=1.5, ls='-')
    ax2.set_yticks(y_pos)
    ax2.set_yticklabels(names)
    ax2.set_xlabel('% Change vs Full Model', fontweight='bold')
    ax2.set_title('B. Component Impact', fontweight='bold', fontsize=12)
    ax2.invert_yaxis()  # Top = most impact
