#!/usr/bin/env python
"""Run the configured phylogeny backend or parse existing summaries."""
...
from stagebridge.pipelines.run_label_repair import run_label_phylogeny


if __name__ == "__main__":
    cfg = compose_config(overrides=["labels=repair"])
    manifest = build_cleaned_cohort_manifest(cfg)["cleaned_manifest"]
    run_label_phylogeny(cfg, manifest=manifest)
