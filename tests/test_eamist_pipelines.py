...
def test_eamist_pretrain_train_evaluate_and_report(tmp_path: Path, monkeypatch) -> None:
    ...
    assert next(benchmark_root.glob("AIS_MIA/*/fold_*/hpo_trial_summary.csv")).exists()
    assert next(benchmark_root.glob("AIS_MIA/*/fold_*/best_hpo_config.json")).exists()
    assert next(benchmark_root.glob("AIS_MIA/*/fold_*/seed_*/selected_hyperparameters.json")).exists()

    checkpoint = next(benchmark_root.glob("AIS_MIA/*/fold_*/seed_*/best_checkpoint.pt"))
