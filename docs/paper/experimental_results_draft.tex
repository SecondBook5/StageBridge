...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/luca_retraining_panel.pdf}
\caption{LuCA scANVI model retraining validation. \textbf{(A)} Training convergence showing ELBO for scVI base model (329 epochs with early stopping) and scANVI label-aware fine-tuning (34 epochs). Train and validation curves demonstrate stable convergence without overfitting. \textbf{(B)} scIB benchmark comparison between retrained and original LuCA models. The retrained model matches or exceeds the original on biological preservation metrics (NMI, ARI, cell type purity) while maintaining batch integration quality. \textbf{(C)} UMAP projections of latent embeddings colored by cell type, comparing original model (overall scIB score 0.669) with retrained model (0.673). Both embeddings preserve cell type structure with comparable separation.}
\label{fig:luca_retraining}
\end{figure}
...
\paragraph{LuCA Mapping.} Initial attempts with only HLCA-derived ENSG mappings yielded only 24.7\% overlap with the LuCA model's 6,000 genes, resulting in out-of-memory failures during gene padding. After incorporating LuCA's own symbol$\to$ENSG mapping, overlap improved to 89.18\%. scArches surgery was performed with identical hyperparameters, with early stopping triggered at epoch 64 (validation ELBO: 1969.0), producing 10-dimensional latent embeddings for all 787,709 query cells.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/embedding_overview.pdf}
\caption{Dual-reference embedding analysis. \textbf{(Top row)} PCA projections of fused (40D), HLCA (30D), and LuCA (10D) embeddings showing complementary structure captured by each reference. \textbf{(Bottom left)} Total variance by reference, with LuCA capturing more variance despite fewer dimensions (10 vs 30), indicating cancer-specific features are more discriminative. \textbf{(Bottom middle)} Per-dimension variance showing contribution of each latent dimension. \textbf{(Bottom right)} HLCA PC1 vs LuCA PC1 correlation ($r = 0.821$), demonstrating that references capture related but non-redundant biological structure.}
\label{fig:embedding_overview}
\end{figure}
...
\midrule
HLCA & 2,000 & 86.05\% & 30 & 58 \\
LuCA & 6,000 & 89.18\% & 10 & 64 \\
\midrule
\textbf{Fused} & -- & -- & 40 & -- \\
\bottomrule
\end{tabular}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/reference_elbo_curves.pdf}
\caption{scArches surgery training convergence for dual-reference mapping. \textbf{(Left)} HLCA mapping: Training ELBO converged rapidly from 733 to 668, with early stopping triggered at epoch 58 when validation ELBO (763.4) ceased improving. \textbf{(Right)} LuCA mapping: Training ELBO converged from 2010 to 1920, with early stopping at epoch 64 (best validation ELBO: 1969.0). Both models demonstrate stable convergence without overfitting, validating the scArches surgical fine-tuning approach for projecting query data onto reference latent spaces.}
\label{fig:reference_elbo}
\end{figure}


\subsection{Spatial Mapping Backend Comparison}

Four spatial mapping backends were evaluated to determine the optimal method for projecting snRNA-seq cells onto tissue coordinates: Tangram \cite{Biancalani2021-bt}, DestVI \cite{Lopez2022-ps}, TACCO, and Cell2location \cite{Kleshchevnikov2022-cell2location}. Each backend was assessed on mapping accuracy using held-out spatial spots with known cell type composition, cell type assignment quality via macro-averaged F1 score, spatial coherence measured by local Moran's I statistic, and computational cost measured as wall-clock runtime on standardized hardware.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/manifold_comparison.pdf}
\caption{Dimensionality reduction of fused dual-reference embeddings (40D) using UMAP, t-SNE, and PHATE, colored by histopathological stage. All three methods reveal clear separation of LUAD cells (dark gray) from precursor stages, with Normal (green), AAH (orange), and AIS (red) showing partial overlap consistent with the gradual nature of early progression. PHATE preserves trajectory-like structure suggesting continuous transitions between stages.}
\label{fig:manifold_comparison}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/fused_umap.pdf}
\caption{UMAP projection of fused dual-reference embeddings colored by disease stage. The embedding captures biologically meaningful structure: Normal tissue (green) and early precursors (AAH, AIS) form interconnected regions, while invasive LUAD (dark gray) segregates into distinct clusters. MIA (purple) occupies intermediate positions consistent with its transitional histopathology.}
\label{fig:fused_umap}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/fused_phate.pdf}
\caption{PHATE projection of fused dual-reference embeddings. PHATE's diffusion-based geometry reveals branching trajectories emanating from central cell populations, potentially capturing multiple progression paths from normal tissue to invasive carcinoma. The trajectory structure suggests heterogeneous evolution with multiple transition routes.}
\label{fig:fused_phate}
\end{figure}
...
Reference mapping (HLCA) & $\sim$46 min & 58 & 1$\times$ H100 GPU \\
Reference mapping (LuCA) & $\sim$64 min & 64 & 1$\times$ H100 GPU \\
