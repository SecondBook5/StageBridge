...
Since reference models expect Ensembl gene identifiers (ENSG) while query data typically uses gene symbols, we first construct a symbol$\to$ENSG mapping by combining annotations from both references. This reconciliation step ensures maximum gene overlap: HLCA provides mappings for its 2,000 highly variable genes, while LuCA contributes mappings for all 17,764 genes in the Core Atlas. The combined mapping enables conversion of $>$85\% of query genes to ENSG identifiers.

For each reference, we apply scArches surgical fine-tuning to pretrained scANVI models. The procedure freezes decoder weights $\theta$ while training a query-specific encoder $\phi'$:
\begin{equation}
