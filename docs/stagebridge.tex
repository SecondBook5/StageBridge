...
Reference tokens $t_i^{(5)}$ and $t_i^{(6)}$ provide direct projections of the HLCA and LuCA embeddings as explicit tokens, allowing the model to attend to reference coordinates independently of the receiver encoding. The pathway token $t_i^{(7)}$ encodes PROGENy pathway activity scores computed for 14 canonical signaling pathways including EGFR, MAPK, PI3K, TGF$\beta$, TNF$\alpha$, and NF$\kappa$B \cite{Schubert2018-ph, Badia-I-Mompel2022-te}. The statistics token $t_i^{(8)}$ encodes neighborhood-level summaries including local cell density, cell-type entropy computed over the type distribution in $\mathcal{N}_i$, and biological covariates such as CAF fraction, immune cell fraction, and diversity indices.
...
Experiments were conducted on NVIDIA H100 NVL GPUs with 256GB system RAM. Training required 13--24 hours per fold depending on early stopping, with the full 5-fold $\times$ 3-seed evaluation totaling approximately 250 GPU-hours.
...
\begin{table}[h]
\caption{Baseline comparison across 5-fold cross-validation with 3 seeds per fold. Validation loss (MSE) measures transition modeling quality; lower is better. StageBridge achieves 16$\times$ lower loss than the best baseline (GraphSAGE) and 22$\times$ lower than simpler pooling methods.}
\label{tab:baselines}
\centering
\begin{tabular}{lcc}
\toprule
...
\midrule
PoolingMLP & 153K & $0.0803 \pm 0.0012$ \\
DeepSets & 153K & $0.0816 \pm 0.0021$ \\
SetTransformer & 153K & $0.0809 \pm 0.0015$ \\
GraphSAGE & 153K & $0.0582 \pm 0.0008$ \\
\textbf{StageBridge} & \textbf{20.5M} & $\mathbf{0.0036 \pm 0.0001}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/publication/fig_training_combined.pdf}
\caption{Training dynamics and baseline comparison. (A) SSL pretraining phase showing train (dashed) and validation (solid) loss on log scale across 5 folds. Loss decreases from $10^{-2}$ to $10^{-7}$ over 50 epochs. (B) Transition training phase showing train and validation loss converging to $\sim$0.0035 MSE. (C) Comparison of StageBridge transition training against baselines---StageBridge converges dramatically faster and to much lower loss. (D) Final validation loss: StageBridge achieves 0.004 MSE versus 0.058 for GraphSAGE (16$\times$ lower) and 0.080--0.082 for simpler methods (22$\times$ lower).}
\label{fig:training}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/publication/fig_ablations.pdf}
\caption{Ablation study results. (A) Violin plots showing validation loss distribution across folds/seeds for each configuration. Green: full model (0.0036 MSE); orange/red: ablations with increasing degradation. The dashed line indicates full model mean. (B) Component impact summary: percent change in validation loss when each component is removed. The gating mechanism shows the largest impact (+11.3\%), followed by GW barycentric fusion (+7.7\%) and HLCA-only reference (+7.2\%). Ring pooling and frozen encoder show negligible impact ($<$1\%).}
\label{fig:ablations}
\end{figure}

\begin{table}[h]
\caption{Ablation study results (5-fold cross-validation, 3 seeds). Validation loss is transition modeling MSE. $\Delta$ indicates percent change versus full model; positive values indicate degradation when component is removed.}
\label{tab:ablation_results}
\centering
\begin{tabular}{lcc}
\toprule
Configuration & Val Loss (MSE) & $\Delta$ \\
\midrule
\textbf{StageBridge (full)} & $\mathbf{0.00356}$ & --- \\
\midrule
\multicolumn{3}{l}{\textit{Architecture ablations}} \\
No gating mechanism & $0.00396$ & $+11.3\%$ \\
No context refiner & $0.00368$ & $+3.5\%$ \\
No distance encoding & $0.00365$ & $+2.6\%$ \\
No niche context & $0.00363$ & $+2.2\%$ \\
No ring pooling & $0.00353$ & $-0.6\%$ \\
Frozen encoder & $0.00353$ & $-0.6\%$ \\
\midrule
\multicolumn{3}{l}{\textit{Reference ablations}} \\
GW barycentric fusion & $0.00383$ & $+7.7\%$ \\
HLCA only & $0.00381$ & $+7.2\%$ \\
LuCA only & $0.00374$ & $+5.3\%$ \\
GW project to HLCA & $0.00371$ & $+4.2\%$ \\
GW project to LuCA & $0.00365$ & $+2.6\%$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[h]
\caption{Full model training results (8 completed runs: 5 folds $\times$ 3 seeds, some runs in progress). Validation loss is transition modeling MSE on held-out donors.}
\label{tab:results}
\centering
\begin{tabular}{lc}
\toprule
Metric & Mean $\pm$ Std \\
\midrule
Validation Loss (MSE) & $0.00356 \pm 0.00010$ \\
Best baseline (GraphSAGE) & $0.0582 \pm 0.0008$ \\
Improvement factor & $16.4\times$ \\
\midrule
\multicolumn{2}{l}{\textit{Key ablation impacts}} \\
No gating & $+11.3\%$ \\
GW barycentric & $+7.7\%$ \\
HLCA only & $+7.2\%$ \\
No niche context & $+2.2\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
Immune composition analysis reveals stage-specific microenvironmental remodeling consistent with prior literature \cite{Sinjab2021-in, Beane2019-mp, Zhao2025-vm}. T cells decrease from 15.9\% in Normal to 5.7\% in Preinvasive tissue, representing a 64\% reduction that precedes frank invasion and may reflect early immune evasion \cite{Chan2026-ov, Hoi2026-jw}. Proliferating cells marked by MKI67 expression increase from 7.1\% in Normal to 25.7\% in Invasive regions, tracking with histopathological progression. Genomic alterations follow expected patterns as shown in Table~\ref{tab:genomic}: TP53 mutations nearly double from 25\% to 48\% across the Normal-to-Invasive spectrum \cite{Zhao2025-vm}, while EGFR mutations increase from 7\% to 29\% and KRAS mutations emerge only in invasive disease (14\%). STK11 mutations are common across all stages (30-39\%), suggesting early-occurring background mutations.
...
\caption{T cell depletion during early progression. Raincloud plot showing T cell fraction per donor across stages. T cells decrease from 15.9\% (Normal) to 5.7\% (Preinvasive), a 64\% reduction ($p < 0.001$), before partial recovery to 10.9\% in Invasive tissue. This immunosuppressive shift precedes frank invasion \cite{Chan2026-ov}.}
\label{fig:tcell}
\end{figure}
...
StageBridge demonstrates that transformer-based niche-conditioned representations dramatically improve cross-sectional transition inference compared to approaches treating cells as isolated expression profiles. The baseline comparison in Table~\ref{tab:baselines} quantifies this effect: StageBridge achieves validation loss of $0.0036 \pm 0.0001$, representing a \textbf{16-fold improvement} over the best baseline (GraphSAGE: $0.058$) and \textbf{22-fold improvement} over PoolingMLP ($0.080$). Notably, PoolingMLP, DeepSets, and SetTransformer all achieve similar performance ($\sim$0.081), suggesting that permutation-invariant aggregation alone provides limited benefit---the key advantage comes from StageBridge's hierarchical ISAB-SAB-PMA architecture with typed spatial tokens. GraphSAGE's explicit graph structure provides modest improvement (0.058), but still falls far short of the receiver-centered attention mechanism. Figure~\ref{fig:umap_flow} visualizes the learned transition dynamics, showing coherent flow fields that connect stage clusters in embedding space.

The ablation study (Table~\ref{tab:ablation_results}) reveals a nuanced picture of component contributions. The \textbf{gating mechanism} emerges as the most critical component, with its removal causing an 11.3\% increase in validation loss---the largest degradation among all ablations. This suggests that the learned gate $g$ in Equation~\ref{eq:drift}, which balances context-conditioned versus latent-only velocity predictions, provides essential flexibility for modeling heterogeneous cell populations where some cells are microenvironment-dependent while others follow cell-autonomous programs.

Surprisingly, the \textbf{niche context} ablation shows only modest degradation (+2.2\%), suggesting that the hierarchical encoder and gating mechanism capture most of the benefit even with limited spatial information. However, this should be interpreted cautiously: the ``no niche'' configuration still includes reference embeddings and pathway tokens, so it ablates spatial ring tokens rather than all contextual information. The \textbf{reference ablations} reveal that dual-reference encoding provides meaningful benefit: using HLCA alone (+7.2\%) or LuCA alone (+5.3\%) both degrade performance compared to the concatenated dual-reference approach, supporting Hypothesis H2 that healthy and disease references provide complementary information.

The Gromov-Wasserstein fusion experiments produced a counterintuitive result: all GW fusion modes \textit{degraded} performance compared to simple concatenation. Barycentric fusion showed the largest degradation (+7.7\%), followed by projection to HLCA (+4.2\%) and projection to LuCA (+2.6\%). This suggests that the geometry-preserving alignment in Equation~\ref{eq:gw}, while theoretically motivated, may introduce distortions when the two reference spaces capture fundamentally different aspects of cellular variation. The simple concatenation in Equation~\ref{eq:concat} allows the downstream attention mechanism to learn which reference dimensions are relevant for each cell type, providing more flexibility than forcing geometric alignment.

Two ablations showed \textit{no degradation}: removing ring pooling ($-0.6\%$) and freezing the encoder ($-0.6\%$). The ring pooling result suggests that the current four-ring spatial discretization may not be optimal---future work should explore continuous distance encoding or adaptive ring boundaries. The frozen encoder result indicates that the pretrained encoder already captures the essential representational structure; fine-tuning during transition learning provides marginal benefit for this dataset size.
...
This work introduced StageBridge, a transformer-based framework for modeling cell state transitions that incorporates local microenvironmental context through receiver-centered niche encoding. The central hypothesis---that spatial neighborhood structure carries information about disease trajectory absent from isolated expression profiles---is supported by empirical results: StageBridge achieves 16$\times$ lower validation loss than the best baseline architecture (0.0036 vs 0.058 for GraphSAGE), with consistent improvements across all five cross-validation folds.

The ablation study revealed that the \textbf{gating mechanism} is the most critical architectural component (+11.3\% degradation when removed), while dual-reference encoding provides meaningful benefit over single-reference approaches (HLCA-only: +7.2\%, LuCA-only: +5.3\%). Interestingly, Gromov-Wasserstein fusion modes all degraded performance compared to simple concatenation, suggesting that allowing the attention mechanism to learn reference weighting outperforms forced geometric alignment. The spatial ring pooling and encoder fine-tuning showed minimal impact, indicating opportunities for architectural simplification in future work.

The architectural contributions that enable these gains include: (1) a hierarchical Set Transformer (ISAB-SAB-PMA) that provides permutation-invariant aggregation with linear complexity through inducing points; (2) typed token embeddings that distinguish receiver, spatial ring, reference, and pathway tokens within unified attention; (3) cross-attention drift conditioning that allows the OT-CFM flow field to query specific niche context when predicting transition velocities; and (4) learned gating between context-conditioned and latent-only velocity pathways. The comparison against global optimal transport methods further revealed a practical scalability advantage: StageBridge's $O(N)$ complexity enables processing of 600K+ cells on standard hardware, whereas methods requiring full transport matrices must subsample due to $O(N^2)$ memory requirements.

The receiver-centered encoding paradigm extends naturally to other spatial biology applications where local context influences cellular phenotype, including tumor-immune interactions \cite{Hoebel2026-tq, Dimitrov2024-fb}, developmental trajectories, and tissue regeneration. As spatial transcriptomics datasets grow to millions of cells, architectures that leverage neighborhood structure while maintaining computational tractability will become essential for extracting biological insight from cross-sectional observations.

\section{Data Availability}

This study integrates multi-modal data from the Peng/Kadara LUAD cohort, all publicly available through the Gene Expression Omnibus: snRNA-seq (798K cells, \href{https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE308103}{GSE308103}), 10x Visium spatial transcriptomics (640K spots, \href{https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE307534}{GSE307534}), and whole-exome sequencing (\href{https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE307529}{GSE307529}). Reference atlases include the Human Lung Cell Atlas (HLCA) \cite{Sikkema2023-dr} and Lung Cancer Atlas (LuCA) \cite{Salcher2022-pu}. Processed neighborhood representations are available at the GitHub repository.

\section{Code Availability}

StageBridge source code, trained model checkpoints, and reproduction scripts are available at \href{https://github.com/SecondBook5/StageBridge}{github.com/SecondBook5/StageBridge} under the MIT license.
